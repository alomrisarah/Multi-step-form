...
const formValidation = () => {
 ...
 formInput.forEach((input) => {
  ...
  if (input.name === "userName") {
   return input.value.length === 0 ? showError(input, "Enter your name") : hideError(input)
  }
  // email
  if (input.name === "email") {
   // verify email input value using regex
   const emailRegExp = new RegExp("[a-z0-9]+@[a-z]+.[a-z]{2,3}")
   return input.value.length === 0 ? showError(input, "Enter email") : !emailRegExp.test(input.value) ? showError(input, "Enter valid email") : hideError(input)
  }
  // phone number
  if (input.name === "phone") {
   return input.value.length === 0 ? showError(input, "Enter your mobile number") : hideError(input)
  }
 })
